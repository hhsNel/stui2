...
static int  begin_color_escape(struct shm_allocator_pdata *pd, struct dblbuf *db);
static int  end_color_escape(struct shm_allocator_pdata *pd, struct dblbuf *db);
static int  move_to(struct shm_allocator_pdata *pd, struct io_buffer *buf, scrcoord x, scrcoord y);
static int  output_char(struct shm_allocator_pdata *pd, struct io_buffer *buf, struct char_cell cc);
static int  output_fg(struct shm_allocator_pdata *pd, struct dblbuf *db, struct char_cell cc);
static int  output_bg(struct shm_allocator_pdata *pd, struct dblbuf *db, struct char_cell cc);
static int  output_cc(struct shm_allocator_pdata *pd, struct dblbuf *db, struct char_cell cc);
...
dump_dblbuf(struct shm_allocator_pdata *pd, struct dblbuf *db, int fd)
{
	...
	return dump_io_buffer(*pd, &db->outbuf, fd);
}

static int
begin_color_escape(struct shm_allocator_pdata *pd, struct dblbuf *db)
{
	if(! db->is_in_color_escape) {
		db->is_in_color_escape = 1;
		return append_io_buffer(pd, &db->outbuf, ANSI_ESC);
	} else {
		return append_io_buffer(pd, &db->outbuf, ";");
	}
}

static int
end_color_escape(struct shm_allocator_pdata *pd, struct dblbuf *db)
{
	if(db->is_in_color_escape) {
		db->is_in_color_escape = 0;
		return append_io_buffer(pd, &db->outbuf, "m");
	}
...
static int
move_to(struct shm_allocator_pdata *pd, struct io_buffer *buf, scrcoord x, scrcoord y)
{
	return printf_io_buffer(pd, buf, ANSI_GOTO("%u","%u"), (unsigned int)y+1, (unsigned int)x+1);
}

static int
output_char(struct shm_allocator_pdata *pd, struct io_buffer *buf, struct char_cell cc)
{
	return write_io_buffer(pd, buf, &cc.c, 1);
}

static int
output_fg(struct shm_allocator_pdata *pd, struct dblbuf *db, struct char_cell cc)
{
	...
	case  CLR_USR:
		if(cc.fg.data.usr < 7) {
			return printf_io_buffer(pd, &db->outbuf, "3%" PRIu8, cc.fg.data.usr);
		} else if (cc.fg.data.usr >= 8 && cc.fg.data.usr < 16) {
			return printf_io_buffer(pd, &db->outbuf, "9%" PRIu8, cc.fg.data.usr - 8);
		}

		return STUI_ERR;
	case  CLR_256:
		return printf_io_buffer(pd, &db->outbuf, "38;5;%" PRIu8, cc.fg.data.c256);
	case  CLR_RGB:
		return printf_io_buffer(pd, &db->outbuf,
		              "38;2;%" PRIu8 ";%" PRIu8 ";%" PRIu8,
		                      cc.fg.data.rgb.r, cc.fg.data.rgb.g, cc.fg.data.rgb.b);
	case  CLR_DEFAULT:
		return append_io_buffer(pd, &db->outbuf, "39");
	case  CLR_UNSET:
		return STUI_OK;
	default:
		return STUI_ERR;
...
static int
output_bg(struct shm_allocator_pdata *pd, struct dblbuf *db, struct char_cell cc)
{
	begin_color_escape(pd, db);
	...
	case  CLR_USR:
		if(cc.fg.data.usr < 7) {
			return printf_io_buffer(pd, &db->outbuf, "4%" PRIu8, cc.fg.data.usr);
		} else if (cc.fg.data.usr >= 8 && cc.fg.data.usr < 16) {
			return printf_io_buffer(pd, &db->outbuf, "10%" PRIu8, cc.fg.data.usr - 8);
		}

		return STUI_ERR;
	case  CLR_256:
		return printf_io_buffer(pd, &db->outbuf, "48;5;%" PRIu8, cc.fg.data.c256);
	case  CLR_RGB:
		return printf_io_buffer(pd, &db->outbuf,
		              "48;2;%" PRIu8 ";%" PRIu8 ";%" PRIu8,
		                      cc.fg.data.rgb.r, cc.fg.data.rgb.g, cc.fg.data.rgb.b);
	case  CLR_DEFAULT:
		return append_io_buffer(pd, &db->outbuf, "49");
	case  CLR_UNSET:
		return STUI_OK;
	default:
		return STUI_ERR;
...
static int
output_cc(struct shm_allocator_pdata *pd, struct dblbuf *db, struct char_cell cc)
{
	if(output_fg(pd, db, cc) != STUI_OK) return STUI_ERR;
	if(output_bg(pd, db, cc) != STUI_OK) return STUI_ERR;
	if(end_color_escape(pd, db) != STUI_OK) return STUI_ERR;
	if(output_char(pd, &db->outbuf, cc) != STUI_OK) return STUI_ERR;
	return STUI_OK;
}
...
redraw_cc(struct shm_allocator_pdata *pd, struct dblbuf *db, scrcoord x, scrcoord y)
{
	if(db->is_redrawing) {
		if(! color_eq(GET_CC(*pd,db->cur_scr,x,y)->fg, db->last_fg)) {
			...
			if(output_fg(pd, db, *GET_CC(*pd,db->cur_scr,x,y)) != STUI_OK) return STUI_ERR;
		}
		if(! color_eq(GET_CC(*pd,db->cur_scr,x,y)->bg, db->last_bg)) {
			db->last_bg = GET_CC(*pd,db->cur_scr,x,y)->bg;
			if(output_bg(pd, db, *GET_CC(*pd,db->cur_scr,x,y)) != STUI_OK) return STUI_ERR;
		}
		if(end_color_escape(pd, db) != STUI_OK) return STUI_ERR;
		return output_char(pd, &db->outbuf, *GET_CC(*pd,db->cur_scr,x,y));
	} else {
		db->is_redrawing = 1;
		if(move_to(pd, &db->outbuf, x, y) != STUI_OK) return STUI_ERR;
		return output_cc(pd, db, *GET_CC(*pd,db->cur_scr,x,y));
	}
}
